...
from hamster_test.comms.constants import INFO_FILE, ERROR_FILE
...
def get_logger():
    ...
    sh2 = logging.FileHandler(filename=INFO_FILE, mode='w', encoding='utf-8')
    sh2.setLevel("INFO")  # 级别是INFO及INFO级别以上的内容

    # 输出 ./error.log文件,并且内容为追加写入,级别是ERROR及ERROR级别以上的内容
    sh3 = logging.FileHandler(filename=ERROR_FILE, mode='w', encoding='utf-8')
